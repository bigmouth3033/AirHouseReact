...
const CalendarViewHost = () => {
  ...
  //min DisabledDate
  const minDisabledDate = new Date(
    Math.min(...bookedDates.map((date) => date.getTime()))
  );
  const maximumn = addDays(new Date(start_date), maximum_stay);
  ...
  return (
    <DateRange
      ...
      minDate={
        isBefore(minDisabledDate, new Date()) ? new Date() : minDisabledDate
      }
