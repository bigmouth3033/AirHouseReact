...
const CalendarViewHost = () => {
  ...
  const   timNgayNhoNhat = (mangNgay) => {
    // Chuyển đổi chuỗi ngày thành đối tượng Date
    const mangNgayDaChuyenDoi = mangNgay.map((ngay) => new Date(ngay));
    
    // Tìm ngày nhỏ nhất
    const ngayNhoNhat = new Date(Math.min.apply(null, mangNgayDaChuyenDoi));
    
    // Chuyển đổi ngày nhỏ nhất thành chuỗi và trả về
    return ngayNhoNhat.toISOString().split('T')[0];
  }
  const handleUserPickRange = (item) => {
    const total = countDay(item);
    if (total[0] >= minimun_stay && total[0] <= maximum_stay) {
      const maximumDate = addDays(new Date(), maximum_stay);
