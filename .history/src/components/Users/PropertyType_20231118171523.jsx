    ...
    const fetchData = async () => {
      try {
        ...
        const responsePropertytpye = await fetch(
          "http://127.0.0.1:8000/api/readPropertyType"
        );
        const resultAmenities = await responseAminities.json();
        const resultPropertyType = await responsePropertytpye.json();
        ...
        setPropertyType(resultPropertyType.data);
      } catch (error) {
  ...
  return (
    <div>
      ...
      <ul>
        ...
        {propertyType.map((properType) => (
          <li key={properType.id}>
            <strong>{properType.name}</strong>
            <p>ID: {properType.id}</p>
            <p>Pic: {properType.icon_image}</p>
            <p>
              Picture: <StyledImage src={properType.icon_image} alt="pic..." />
            </p>
            {/* Hiển thị các thông tin khác về amenity nếu cần */}
            <br />
            <br />
