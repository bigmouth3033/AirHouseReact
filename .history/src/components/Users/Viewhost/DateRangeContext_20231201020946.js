...
export const DateRangeProvider = ({ children }) => {
  ...
  const maxEndDay = (startDay) => {
    // Tìm ngày cuối cùng trong bookedDates mà lớn hơn startDay
    const relevantBookedDates = bookedDates.filter(
      (bookedDate) => startOfDay(bookedDate).getDate() > startDay
    );

    if (relevantBookedDates.length > 0) {
      const maxDate = Math.max(
        ...relevantBookedDates.map((date) => date.getDate())
      );
      return maxDate;
    }
  ...
  const disableBookedDates = ({ date }) => {
    ...
    const endDayLimit = maxEndDay(startDay);

    if (endDayLimit !== null) {
      // Nếu có giới hạn endDay, vô hiệu hóa các ngày sau endDayLimit
      return date.getDate() > endDayLimit || isDateBooked(date);
    }
