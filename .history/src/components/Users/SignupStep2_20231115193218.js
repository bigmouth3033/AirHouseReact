...
const SignupStep2 = () => {
  ...
  const validateBirthday = (e) => {
    ...
    if (isNaN(selectedDate.getTime())) {
      e.preventDefault();
      const err = setErrorText("Invalid date of birth.");
      alert(err);
    } else if (age < 18) {
      e.preventDefault();
      const err = setErrorText("You must be at least 18 years old.");
      alert(err);
    } else if (selectedDate > today) {
      e.preventDefault();
      const err = setErrorText("Date of birth cannot be in the future.");
      alert(err);
    } else if (name.trim() === "") {
      e.preventDefault();
      const err = setErrorText("Name is required.");
      alert(err);
    } else if (!/^[a-zA-Z ]+$/.test(name)) {
      e.preventDefault();
      const err = setErrorText(
      ...
      <StyledForm>
        <form>
          <StyledFormContainer>
            ...
            />
            {/* <StyledSpan>{errorText}</StyledSpan> */}
          ...
          <StyledButtonSubmit type="submit" onClick={validateBirthday}>
            Continute
          </StyledButtonSubmit>
        </form>
